...
python -c "import os; import certifi; from dotenv import load_dotenv; from pymongo import MongoClient; load_dotenv(); uri = os.getenv('MONGO_URI', 'mongodb://localhost:27017/party_db'); client = MongoClient(uri, serverSelectionTimeoutMS=5000, tls=(uri.startswith('mongodb+srv')), tlsCAFile=certifi.where() if uri.startswith('mongodb+srv') else None); client.admin.command('ping'); print('✅ Database connection verified')"
if %errorlevel% neq 0 (
    echo.
